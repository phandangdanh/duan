  ...
  if (quantity > availableStock) {
    if (availableStock <= 0) {
      if (stockType === 'main') {
        ...
        showNotification('Biến thể này đã hết hàng! Vui lòng chọn màu và size khác.', 'error');
      }
    } else {
    ...
    if (availableStock <= 0) {
      ...
      if (stockType === 'main') {
        ...
        if (hasAnyAvailableVariant) {
          ...
          showNotification('Tất cả biến thể đã hết hàng! Bạn có thể mua sản phẩm chính.', 'info');
        }
        return false;
      }
    } else {
...
function autoSelectAvailableVariant() {
  ...
  if (!productVariants || productVariants.length === 0) {
    ...
    showNotification('Không có biến thể nào có sẵn!', 'error');
    return false;
  }
  ...
  if (availableVariants.length === 0) {
    ...
    showNotification('Tất cả biến thể đã hết hàng!', 'error');
    return false;
  }
...
function updateVariantSelection(variant) {
  ...
  showNotification(`Đã chuyển sang ${variant.color_name || 'màu'} ${variant.size_name || 'size'} có sẵn (${variant.stock} sản phẩm)`, 'success');
}
