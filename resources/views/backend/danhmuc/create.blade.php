            ...
            <div class="col-lg-12">
                <div class="ibox float-e-margins">
                    ...
                    <div class="ibox-content">
                        ...
                        <form action="{{ route('danhmuc.store') }}" method="POST" enctype="multipart/form-data" class="form-horizontal">
                            @csrf
                            ...
                            <div class="form-group {{ $errors->has('parent_id') ? 'has-error' : '' }}">
                                ...
                                <div class="col-sm-10">
                                    <select name="parent_id" class="form-control {{ $errors->has('parent_id') ? 'is-invalid' : '' }}">
                                        <option value="0" {{ old('parent_id', '0') == '0' ? 'selected' : '' }}>Danh mục gốc</option>
                                        @php
                                            $renderOptions = function($items, $level = 0) use (&$renderOptions) {
                                                foreach ($items as $item) {
                                                    $prefix = str_repeat('— ', $level);
                                                    echo '<option value="' . $item->id . '"' . (old('parent_id') == $item->id ? ' selected' : '') . '>' . $prefix . e($item->name) . '</option>';
                            ...
                            <div class="form-group {{ $errors->has('image') ? 'has-error' : '' }}">
                                <label class="col-sm-2 control-label">Ảnh danh mục</label>
                                <div class="col-sm-10">
                                    <input type="file" name="image" class="form-control {{ $errors->has('image') ? 'is-invalid' : '' }}" 
                                           accept="image/*">
                                    <small class="text-muted">
                                        <i class="fa fa-info-circle"></i> Chấp nhận: JPG, PNG, GIF. Tối đa: 2MB
                                    </small>
                                    @error('image')
                                        <span class="help-block text-danger">
                                            <i class="fa fa-exclamation-triangle"></i> {{ $message }}
                            ...
                            <div class="form-group {{ $errors->has('sort_order') ? 'has-error' : '' }}">
                                ...
                                <div class="col-sm-10">
                                    <input type="number" name="sort_order" class="form-control {{ $errors->has('sort_order') ? 'is-invalid' : '' }}" 
                                           value="{{ old('sort_order', 0) }}" min="0" placeholder="0" title="Ưu tiên hiển thị (0 là mặc định)">
                            ...
                            <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                                ...
                                <div class="col-sm-10">
                                    ...
                                    <div class="radio">
                                        <label>
                                            <input type="radio" name="status" value="inactive" {{ old('status', 'active') == 'inactive' ? 'checked' : '' }}>
