...
class VoucherUpdateRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            'ma_voucher' => [
                ...
                'string',
                'max:50',
                Rule::unique('voucher', 'ma_voucher')->ignore($voucherId),
                'regex:/^[A-Z0-9_]+$/'
            ],
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            $voucher = \App\Models\Voucher::find($this->route('voucher'));
            if ($voucher && $this->so_luong < $voucher->so_luong_da_su_dung) {
                $validator->errors()->add('so_luong', 'Số lượng voucher mới không được nhỏ hơn số lượng đã sử dụng (' . $voucher->so_luong_da_su_dung . ')');
            }
        });
    }
}
